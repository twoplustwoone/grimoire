...
export default async function LocationDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {notes.length > 0 && (<Card className="mb-4"><CardHeader><CardTitle className="text-base">Notes</CardTitle></CardHeader><CardContent><div className="space-y-3">{notes.map((n) => (<div key={n.id} className="text-sm border-l-2 pl-3"><p><MentionRenderer content={n.content} campaignId={campaignId} /></p><p className="text-xs text-muted-foreground mt-1">{new Date(n.createdAt).toLocaleDateString()}</p></div>))}</div></CardContent></Card>)}
